...
<div class="sm:block flex flex-grow md:hidden ml-4 relative">
	...
	{#if showMobileEditToolbar}
		<div class="fixed flex flex-col px-4 py-1 shadow-sm text-sm">
			<button
...
<div class="px-2 py-1 flex-grow hidden md:flex">
	<button
		class="px-2 rounded-lg flex items-center text-sm bg-transparent"
		on:click={() => {
			dispatch('edit', { mode: 'add' });
			isEditModeActive = true;
		}}><PlusOutline size="sm" />Add</button
	>
	<button class="px-2 rounded-lg flex items-center text-sm bg-transparent"
		><PenOutline
			size="sm"
			on:click={() => {
